...
class MeowfficerEnhance(MeowfficerBase):
    ...
    def _meow_enhance(self):
        ...
        if coins < 1000:
            logger.info(f"Coins ({coins}) < 1000. Not enough coins to complete " f"enhancement, skip")
            return "coin_limit"
        ...
        while 1:
            ...
            coins = MEOWFFICER_COINS.ocr(self.device.image)
            if coins < 1000:
                logger.info(f"Remaining coins ({coins}) < 1000. Not enough coins for next " f"enhancement, skip")
                break
